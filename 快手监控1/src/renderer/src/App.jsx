...
import { Activity, TrendingUp, RefreshCw, Clock, Search, Settings, Zap } from 'lucide-react'
...
// --- Column Definitions ---
// 重点修改：align 全部改为 'center' (除了用户列)
const COLUMNS = [
  { label: '用户', key: null, width: 'w-64', align: 'left' },
  { label: 'GMV', key: 'GMV', width: 'w-32', align: 'center' }, // 居中
  { label: '上次', key: '上次花费', width: 'w-28', align: 'center' }, // 居中
  { label: '花费', key: '花费', width: 'w-28', align: 'center' }, // 居中
  { label: '消耗', key: '消耗', width: 'w-28', align: 'center' }, // 居中
  { label: '全站ROI', key: '全站ROI', width: 'w-28', align: 'center' }, // 居中
  { label: '投放ROI', key: 'roi', width: 'w-28', align: 'center' }, // 居中
  { label: '操作', key: null, width: 'w-56', align: 'center' } // 居中
]
            ...
            <Card className="flex flex-wrap items-center justify-between p-5 gap-6 shrink-0 shadow-md">
              <div className="flex items-center gap-8">
                <div>
                  ...
                  {/* 修改：加大更新时间的字号 (text-xs -> text-base) 和图标大小 */}
                  {lastUpdateTime && (
              ...
              <div className="overflow-auto flex-1">
                ...
                <table className="w-full text-left border-collapse table-fixed">
                  <thead className="bg-zinc-50/95 sticky top-0 z-10 backdrop-blur-sm border-b border-zinc-200">
                    <tr>
                      {COLUMNS.map((col, i) => (
                        <th
                          key={i}
                          // 修改：表头字体加大 text-base (16px)
                          className={`px-5 py-4 text-base font-bold text-zinc-600 uppercase tracking-wide ${col.width} ${col.key ? 'cursor-pointer hover:text-blue-600 select-none transition-colors' : ''} ${col.align === 'right' ? 'text-right' : col.align === 'center' ? 'text-center' : 'text-left'}`}
                  ...
                  <tbody className="divide-y divide-zinc-100">
                      ...
                      return (
                        <tr key={row.UID} className="hover:bg-blue-50/40 transition-colors group">
                          ...
                          <td className="px-5 py-3 text-left">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-xl bg-zinc-100 overflow-hidden border border-zinc-200 shrink-0 shadow-sm group-hover:shadow-md transition-all">
                                {row.头像 && (
                                  <img
                              ...
                              <div className="min-w-0 flex flex-col justify-center">
                                <div
                                  className="font-medium text-base text-zinc-800 truncate max-w-[160px]"
                                  title={row.名称}
                                >
                                  {row.名称}
                                </div>
                          ...
                          {/* 数字列：全部统一居中对齐 (align='center') + 字体加大 text-base (16px) */}
                          <td className="px-5 py-3 font-mono text-base text-zinc-800 text-center">
                            ¥{row.GMV}
                          </td>
                          <td className="px-5 py-3 font-mono text-base text-zinc-800 text-center">
                            {row.上次花费}
                          ...
                          {/* ROI：居中对齐 */}
                          <td className="px-5 py-3 font-mono text-base font-bold text-zinc-800 text-center">
                            {row.全站ROI}
                          </td>
                          <td className="px-5 py-3 font-mono text-base font-bold text-blue-00 text-center">
                            {row.roi}
                          </td>
                          ...
                          <td className="px-5 py-3 text-center">
                            <div className="flex items-center justify-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                              <Input
                                size="md"
                                className="w-24 text-center font-mono bg-white shadow-sm"
                                placeholder={row.roi}
                                value={row.editRoi || ''}
                    ...
                    {sortedData.length === 0 && (
                      <tr>
                        <td colSpan={8} className="p-16 text-center text-zinc-400 text-base">
